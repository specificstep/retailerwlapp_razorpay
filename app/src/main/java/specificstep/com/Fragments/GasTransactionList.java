...
public class GasTransactionList extends Fragment implements View.OnClickListener, SearchListAdapter.SubmitComplainClickListener {
    ...
    private Context getContextInstance() {
        if (context == null) {
            context = GasTransactionList.this.getActivity();
            return context;
        } else {
            return context;
        }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (Constants.serviceModelArrayList != null && Constants.serviceModelArrayList.size() > 0) {
            for (int i = 0; i < Constants.serviceModelArrayList.size(); i++) {
                if (Constants.serviceModelArrayList.get(i).getName().equalsIgnoreCase(Constants.KEY_GAS_TEXT)) {
                    service_id = Constants.serviceModelArrayList.get(i).getId();
                }
            }
        } else {
            service_id = Constants.gas_id;
        }
...
//        },500);
        ...
        searchListAdapter = new SearchListAdapter(getContextInstance(), rechargeArrayList, this, Constants.KEY_GAS_TEXT);
    ...
    public void setUserVisibleHint(final boolean isUserVisible) {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                //if (isUserVisible && isResumed() && isAdded() && getUserVisibleHint()) {
                if (isUserVisible && isResumed()/* && isResumed() && getUserVisibleHint()*/) {
                    start = 0;
                    str_mo_no = "";
                    strMobile = "";
                    strMonth = 0;
    ...
    public void showFilter() {
        ...
        imgEditText.setImageResource(R.drawable.gas);
