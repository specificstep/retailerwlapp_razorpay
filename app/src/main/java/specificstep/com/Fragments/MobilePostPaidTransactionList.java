 ...
 */
public class MobilePostPaidTransactionList extends Fragment implements View.OnClickListener, SearchListAdapter.SubmitComplainClickListener {
    ...
    private Context getContextInstance() {
        if (context == null) {
            context = MobilePostPaidTransactionList.this.getActivity();
            return context;
        } else {
            return context;
        }
    ...
    public MobilePostPaidTransactionList() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_trans_search, container, false);
        ...
        if (Constants.serviceModelArrayList != null && Constants.serviceModelArrayList.size() > 0) {
            for (int i = 0; i < Constants.serviceModelArrayList.size(); i++) {
                if (Constants.serviceModelArrayList.get(i).getName().equalsIgnoreCase(Constants.KEY_MOB_POSTPAID_TEXT)) {
                    service_id = Constants.serviceModelArrayList.get(i).getId();
                }
            }
        } else {
            service_id = Constants.mobile_postpaid_id;
        }
        ...
        searchListAdapter = new SearchListAdapter(getContextInstance(), rechargeArrayList, this, Constants.KEY_MOB_POSTPAID_TEXT);
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                if (isUserVisible && isResumed()/* && isResumed() && isAdded() && getUserVisibleHint()*/) {
                    ...
                    strYear = 0;
                    str_month_year = "";
                    Dlog.d("Mobile Postpaid User call");
                    makeNativeTransactionSearch();
                }
            }
        },500);
