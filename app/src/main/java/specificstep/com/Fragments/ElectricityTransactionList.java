...
public class ElectricityTransactionList extends Fragment implements View.OnClickListener, SearchListAdapter.SubmitComplainClickListener {
    ...
    private Context getContextInstance() {
        if (context == null) {
            context = ElectricityTransactionList.this.getActivity();
            return context;
        } else {
            return context;
        }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (Constants.serviceModelArrayList != null && Constants.serviceModelArrayList.size() > 0) {
            for (int i = 0; i < Constants.serviceModelArrayList.size(); i++) {
                if (Constants.serviceModelArrayList.get(i).getName().equalsIgnoreCase(Constants.KEY_ELECTRICITY_TEXT)) {
                    service_id = Constants.serviceModelArrayList.get(i).getId();
                }
            }
        } else {
            service_id = Constants.electricity_id;
        }
        ...
        searchListAdapter = new SearchListAdapter(getContextInstance(), rechargeArrayList, this, Constants.KEY_ELECTRICITY_TEXT);
    ...
    public void showFilter() {
        ...
        imgEditText.setImageResource(R.drawable.ic_electricity2);
